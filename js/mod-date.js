...
window.addEventListener("DOMContentLoaded", () => {
    if (window.variableObserver) {
        // Suscribirse a los cambios de ax y bx
        window.variableObserver.subscribe(({ ax, bx }) => {
            console.log("Variables recibidas en mod-date.js:", { ax, bx });
                ...
                canasta_year_select.addEventListener("change", function () {
                    ...
                    if (selectedYear == yearGet) {
                        ...
                        for (let i = monthGet; i <= 12; i++) {
                            document
                                .querySelector(`.canasta_month_option[value='${i}']`)
            ...
            function get_table_past(ax, bx) {
                ...
                suma_indigencia_past = alquiler_past_value + Math.round(ax * cba_Old);
                suma_pobreza_past = alquiler_past_value + Math.round(ax * cbt_Old);
                suma_clase_baja_fragil_past = alquiler_past_value + Math.round(ax * cbt_Old * 1.2);
                suma_clase_baja_past = alquiler_past_value + Math.round(ax * cbt_Old * 1.5);
                suma_clase_media_fragil_past = alquiler_past_value + Math.round(ax * cbt_Old * 2);
                suma_clase_media_past = alquiler_past_value + Math.round(ax * cbt_Old * 5);
                suma_clase_media_alta_past = alquiler_past_value + Math.round(ax * cbt_Old * 8);
                suma_clase_alta_baja_past = alquiler_past_value + Math.round(ax * cbt_Old * 12);
        ...
        console.error("variableObserver no está definido");
    }
});
